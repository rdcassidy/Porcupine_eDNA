...
ASV_table <- readr::read_csv("run2/filtered_ASV_table.csv")
hash.fasta <- readDNAStringSet("run2/Filtered_Hash_key.fasta")
...
ASV_table_unique <- ASV_table %>%
  ...
  tidyr::unite(new_name,c(1:2),sep = ";size=", remove=F) %>%
  select(-total_reads) %>%
...
rename.fasta(infile = "run2/Filtered_Hash_key.fasta",ref_table = ASV_table_unique,outfile = "run2/hash_key_for_swarm_04.02.2024.fasta")
...
swarm.fasta <- readDNAStringSet("C:\Users\Usuario\Dropbox\Porcupain_project\run2/output_centro.04.02.2024.fasta")
...
rename.fasta(infile = "C:\Users\Usuario\Dropbox\Porcupain_project\run2/output_centro.04.02.2024.fasta",ref_table = names.for.vsearch,outfile = "C:\Users\Usuario\Dropbox\Porcupain_project\run2/hash_key_for_vsearch.04.02.2024.fasta")
...
satelites <- read.table("C:\Users\Usuario\Dropbox\Porcupain_project\run2/swarm_output04.02.2024.txt", sep=" ", fill=TRUE,header=FALSE) %>%
  filter(!is.na(V2) & V2 != "") %>%
  mutate_all(~gsub(";.*", "", .))
...
ASV_table <- read_csv("C:\Users\Usuario\Dropbox\Porcupain_project\run2/filtered_ASV_table.csv")
...
planestas.fasta <- readDNAStringSet("C:\Users\Usuario\Dropbox\Porcupain_project\run2/output_centro.04.02.2024.fasta")
...
write.csv(summary_data,here("C:\Users\Usuario\Dropbox\Porcupain_project\run2/ASV_table_after_swarm04.02.2024.csv"), row.names = FALSE)
...
chimeras <- readDNAStringSet("C:\Users\Usuario\Dropbox\Porcupain_project\run2//vsearch_chimeras.fasta")
...
write.csv(ASV_table_after_vsearch,"C:\Users\Usuario\Dropbox\Porcupain_project\run2/ASV_table_after_vsearch.04.02.2024.csv", row.names = FALSE)
...
all.hashes <- readDNAStringSet("C:\Users\Usuario\Dropbox\Porcupain_project\run2/Filtered_Hash_key.fasta")

hashes.after.clean <- read.csv("C:\Users\Usuario\Dropbox\Porcupain_project\run2/ASV_table_after_vsearch.04.02.2024.csv")
hashes.after.clean <- unique(hashes.after.clean$Hash)

cleaned.hashes <- all.hashes[hashes.after.clean]

writeXStringSet(cleaned.hashes, file = "~/Dropbox/Porcupain_project/run2/hashes_after_vsearch.04.02.2024.fasta", format = "fasta")
...
all.hashes <- read.csv(here("C:\Users\Usuario\Dropbox\Porcupain_project\run2/filtered_hash_key.csv"))

final.hashes <- read.csv(here("C:\Users\Usuario\Dropbox\Porcupain_project\run2/ASV_table_after_vsearch.04.02.2024.csv"))
...
write.csv(hash.key.after.vsearch, here("C:\Users\Usuario\Dropbox\Porcupain_project\run2/hash_key_after_vsearch.04.02.2024.csv"), row.names = F)
